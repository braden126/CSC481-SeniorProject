...
*Note 2: A Moodle session is defined as the time from the first action taken in a session to the last action taken. If a student more than an hour is between the next action, the previous session ends and a new session begins. An hour was chosen since students may be spending a long time viewing resources on a Moodle page without return to a new home page.*
```{r, message=FALSE, warning=FALSE, echo=FALSE}
...
Here, sessions less than 10 minutes are removed to get a closer look at the medium to long length sessions. The highest two amounts were 20 and 25 minutes sessions. After reaching 60 minute sessions, the number of sessions longer than 60 decreases rapidly. 
...
```
Returning to the time and week visualization with amount of sessions instead we can see when students are having short and long Moodle sessions.The first item that jumps out is that a majority of the long Moodle sessions during times when their are classes going on. Outside of class times, only sunday and wednesday night are when a noticeable amount of long sessions occur similar to the previous time and week visualization.
...
```
The most common event in a session was overwhelmingly course viewed. The next most common was checking an assignment and viewing the grade report.

```{r, message=FALSE, warning=FALSE, echo=FALSE}

## idk if this is something to keep
