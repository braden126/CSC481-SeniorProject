...
The objective of this analysis is to research how effective the new plug-ins were that are being tested during the Spring 2021 semester and research how students are using Moodle during this year of hybrid. The goal is for this analysis to give faculty and instructors and better idea of how students are using Moodle and how they can structure classes that work best with students.
...
The diverse levels of submission times between classes suggest that class structure may have a strong impact on how long students spend between viewing ann assignment and submitting it in but there is not quite enough data here to confirm that assumption.
...
After 14:00, activity steadily drops until 12:00 suggesting further that this is when a large amount of students start going on Moodle again after class hours. There is also a larger number of assignment submissions starting at 20:00 until 01:00 the next day.
...
Similarly looking at totals by day of the week, the activity of a week peaks on Yuesday and then falls each day until Sunday.
...
*Note 2: A Moodle session's length is defined as the time from the first action taken in a session to the last action taken. If there is more than an hour between a student's last two actions, the current session ends and a new session begins. An hour was chosen because students may be spending a long time viewing a single resource in a module, creating no new log files despite them being on Moodle for an extended period of time.*
...
Here, sessions less than 10 minutes are removed to get a closer look at the medium to long length sessions. The highest two amounts were 20 and 25 minutes sessions. After reaching 60 minute sessions, the number of sessions longer than 60 decreases rapidly. This suggests that when students are doing work on Moodle, most will either switch to a working on a different course or stop using Moodle after about 60 minutes during long sessions. 
...
Although there were students that almost completely ignored the checklists, most used the checklist. The average number of views plus checks was almost 30 and a decent number of students used the checklist extensively.
...
While only the Perusall and Checklist plug-ins were captured by the logs, those plug-ins combined the plug-ins that could not be captured by the logs did seem to have an effect on the test group of classes. 
